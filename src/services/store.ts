...
import { DataBoardWithDetails, Store } from '@app/types';

const useStore = createStore<Store>(set => ({
  boardsWithDetails: [],

  fetchBoardsWithDetails: async (boardId?: DataBoardWithDetails['id']) => {
    return await new Promise(resolve => {
      setTimeout(() => {
        if (boardId !== undefined) {
          ...
          const boardsWithDetails: DataBoardWithDetails[] = [
